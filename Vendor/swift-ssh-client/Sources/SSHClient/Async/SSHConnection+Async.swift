...
public extension SSHConnection {
    ...
    func requestShell(pty: SSHPseudoTerminal,
                      withTimeout timeout: TimeInterval? = nil) async throws -> SSHShell {
        try await withTaskCancellationHandler { completion in
            requestShell(pty: pty, withTimeout: timeout, completion: completion)
        }
    }
