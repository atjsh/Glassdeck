...
enum SSHShellEvent {
    case requestStart(Channel, Promise<Void>, SSHPseudoTerminal?)
    case requestWrite(Data, Promise<Void>)
    case requestResize(SSHWindowSize, Promise<Void>)
...
enum SSHShellAction {
    ...
    case close(Channel)
    case start(Channel, SSHPseudoTerminal?)
    case resize(SSHWindowSize, Channel, Promise<Void>)
...
struct SSHShellStateMachine {
    ...
    mutating func handle(_ event: SSHShellEvent) -> SSHShellAction {
        ...
        case .starting(let channel, let promise):
            ...
            case .requestStart(_, let promise, _):
                return .callPromise(promise, .failure(SSHShellError.requireConnection))
            case .requestWrite(_, let promise):
                return .callPromise(promise, .failure(SSHShellError.requireConnection))
            case .requestResize(_, let promise):
        ...
        case .ready(let channel):
            ...
            case .requestResize(let size, let promise):
                return .resize(size, channel, promise)
            case .requestClosing(let promise):
                internalState = .closing(promise)
                return .close(channel)
            case .started:
