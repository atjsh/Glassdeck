...
public class SSHConnection: @unchecked Sendable {
    ...
    public func requestShell(withTimeout timeout: TimeInterval? = nil,
        ...
        requestShell(startupPTY: nil, withTimeout: timeout, queue: queue, completion: completion)
    }

    @discardableResult
    public func requestShell(pty: SSHPseudoTerminal,
                             withTimeout timeout: TimeInterval? = nil,
                             queue: DispatchQueue? = nil,
                             completion: @escaping (Result<SSHShell, Error>) -> Void) -> SSHTask {
        requestShell(startupPTY: pty, withTimeout: timeout, queue: queue, completion: completion)
    }

    @discardableResult
    private func requestShell(startupPTY: SSHPseudoTerminal?,
                              withTimeout timeout: TimeInterval? = nil,
                              queue: DispatchQueue? = nil,
                              completion: @escaping (Result<SSHShell, Error>) -> Void) -> SSHTask {
        let shell = SSHShell(
