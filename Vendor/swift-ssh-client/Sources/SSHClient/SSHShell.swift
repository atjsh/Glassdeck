...
public class SSHShell: @unchecked Sendable, SSHSession {
    ...
    private let startupPTY: SSHPseudoTerminal?
    ...
    init(ioShell: IOSSHShell,
         ...
         startupPTY: SSHPseudoTerminal?) {
        self.ioShell = ioShell
        self.updateQueue = updateQueue
        self.startupPTY = startupPTY
        setupIOShell()
    }
    ...
    func start(in context: SSHSessionContext) {
        ioShell.start(in: context, pty: startupPTY)
    }
    ...
    public func resize(to size: SSHWindowSize,
                       completion: @escaping (Result<Void, Error>) -> Void) {
        ioShell.resize(to: size).whenComplete(on: updateQueue, completion)
    }
